# PowerShell Build Script for Project Opener Extension
# Builds EXE installers for x64 and ARM64 architectures
...
$ProjectDir = "$PSScriptRoot\$ExtensionName"
...
foreach ($Platform in $Platforms) {
    ...
    $setupTemplate = Get-Content "$PSScriptRoot\setup-template.iss" -Raw
    ...
    $setupScript | Out-File -FilePath "$PSScriptRoot\setup-$Platform.iss" -Encoding UTF8
    ...
    if (Test-Path $InnoSetupPath) {
        & $InnoSetupPath "$PSScriptRoot\setup-$Platform.iss"
        ...
        Write-Warning "Inno Setup not found. Please install from https://jrsoftware.org/isdl.php"
        Write-Host "Expected location: $InnoSetupPath" -ForegroundColor Yellow
    }
}

Write-Host "`n🎉 Build completed!" -ForegroundColor Green
Write-Host "Installers location: $ProjectDir\bin\$Configuration\installer\" -ForegroundColor Cyan
